...
Magit depends on the ~dash~, ~lv~, ~transient~ and ~with-editor~ libraries
...
If you haven't installed ~dash~, ~lv~, ~transient~ and ~with-editor~ from
...
value of ~magit-use-sticky-arguments~.

- User Option: magit-use-sticky-arguments

  This option controls how the diff and log transients reuse arguments
  from existing buffers.

  When ~t~ (the default value), then the log or diff transients reuses
  ...
  When ~current~, log and diff transients will only reuse the arguments
...
~--follow~.  In this case, the ~-u~ argument of the log transient can be
...
- Key: Mt, git-rebase-merge-toggle-editmsg
  ...
  This command toggles the ~-C~ and ~-c~ options of the merge action
  at point.  These options both specify a commit whose message should
  be reused.  The lower-case variant instructs Git to invoke the
  editor when creating the merge, allowing the user to edit the
  message.
